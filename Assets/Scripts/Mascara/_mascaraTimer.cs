...
public class _mascaraTimer : MonoBehaviour
{
    Image BarraTiempo;
    ...
    public void Ganar()
    {
        ...
        AuidoScript.instance.Play("CerdoFinal");
        Jugando = false;
    ...
    {
        int bandera = PlayerPrefs.GetInt("mascaraFlag");
        bandera++;
        PlayerPrefs.SetInt("mascaraFlag", bandera);

        if (PlayerPrefs.GetInt("mascaraFlag") >= 5)
        {
        ...
        AuidoScript.instance.Mute("Marcha");
        FindObjectOfType<_mascaraManager>().Perder();
